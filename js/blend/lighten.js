chroma.blend.lighten = {};

/**
 * The final color is a color composed of the lightest values per color
 * channel.
 *
 ...
 */
chroma.blend.lighten.apply = function(bottom_canvas, top_canvas) {
  for (var y = 0; y < bottom_canvas.length; y++) {
    for (var x = 0; x < bottom_canvas[y].length; x++) {
      if (bottom_canvas[y][x] === null) {
        ...
        bottom_canvas[y][x] = chroma.rgb(
          Math.max(bottom_canvas[y][x].r, top_canvas[y][x].r),
          Math.max(bottom_canvas[y][x].g, top_canvas[y][x].g),
          Math.max(bottom_canvas[y][x].b, top_canvas[y][x].b)
        );
      }
    }
  }
